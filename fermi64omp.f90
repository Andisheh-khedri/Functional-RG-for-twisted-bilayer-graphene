!-----------------------------------------------------------------------------------------------------
! *****************--compared to fermi63.f90 trying to employ omp to distribute jobs
!-----------------------------------------------------------------------------------------------------
module commons
INTEGER, PARAMETER::nspin=2, norbital=2, ntype=2, nband=nspin*norbital*ntype&  !
                   ...
                   ,idpermute=0,IdFSrotsym=0,Idantisym=0,Idantisymextra=0,ISOC=0,Idbandsym=1
...
!------------------------------------------------------------------
DOUBLE PRECISION,allocatable,DIMENSION(:)::Yvertexre,YDvertexre
...
!-----------------------------------------------------------------------------------------------------
!-----------------------------------------------------------------------------------------------------
open(unit=1,file="Aug1testa1r1b1np12omp.txt",status="new",action="write",position="rewind",IOSTAT=openstatus)
!-----------------------------------------------------------------------------------------------------
!-----------------------------------------------------------------------------------------------------
...
!----------------------------------------------------------------------
allocate(jmat(1:nband,1:nband,1:nband,1:nband,1:npatch,1:npatch,1:npatch))
allocate(n1mat(1:Ntotal),n2mat(1:Ntotal),n3mat(1:Ntotal),n4mat(1:Ntotal),p1mat(1:Ntotal),p2mat(1:Ntotal),p3mat(1:Ntotal))
allocate(p4vec(1:nband,1:nband,1:nband,1:npatch,1:npatch,1:npatch,1:2))
allocate(Yvertexcomplex(1:Ntotal),YvertexABinitial(1:Ntotal))
allocate(Yvertexre(1:Ntot),YDvertexre(1:Ntot))
!----------------------------------------------------------------------
!----------------------------------------------------------------------
!if(pi<0)then
!----------------------------------------------------------------------
...
deallocate(Yvertexre,YDvertexre)
!-----------------------------------------------------------------------
end program HFRG
!------------------------------------------------------------------------------------------------------------
!------------------------------------------------------------------------------------------------------------
...
INTEGER,DIMENSION(1:NEQ):: nrep
!INTEger,DIMENSION(1:nband,1:nband,1:nband,1:nband,1:npatch,1:npatch,1:npatch)::imat
INTEGER,DIMENSION(5:8,5:8,5:8,5:8,1:npatch,1:npatch,1:npatch)::imat
INTEGER,allocatable,DIMENSION(:,:,:,:,:,:,:)::imatshadow
INTEger,DIMENSION(1:nband,1:nband,1:nband,1:npatch,1:npatch,1:npatch)::p4int
INTEger,allocatable,DIMENSION(:,:,:,:,:,:)::p4intshadow
!-------------------------------------------------------------------------------------------------------
...
INTEGER,dimension(1:npatch)::p4th,ith
...
!----------------------------------------------------
allocate(p4intshadow(1:nband,1:nband,1:nband,1:npatch,1:npatch,1:npatch))
allocate(imatshadow(5:8,5:8,5:8,5:8,1:npatch,1:npatch,1:npatch))
!------------------------------------------------------------------
p4intshadow(1:nband,1:nband,1:nband,1:npatch,1:npatch,1:npatch)=p4int(1:nband,1:nband,1:nband,1:npatch,1:npatch,1:npatch)
imatshadow(5:8,5:8,5:8,5:8,1:npatch,1:npatch,1:npatch)=imat(5:8,5:8,5:8,5:8,1:npatch,1:npatch,1:npatch)
!-------------------------------------------------------------------------------------------------------
!-------------------------------------------------------------------------------------------------------
scalecommon=x
icall=icall+1
...
!----------------------------------------------------------------------
! Righ-hand side of flow:
!----------------------------------------------------------------------
!$OMP PARALLEL SHARED(Yvertexre,nrep,YDvertexre,maxvertex,taupp,tauphd,tauphc,NEQ) , &
!$OMP& PRIVATE(n1,n2,n3,n4,n2max,p1,p2,p3,p4,p3max,i,j1,j2,j3,j4,n1r,n2r,n3r,l1r,l2r,k,l1,l2,sint) , &
!$OMP& SHARED(Matsumpp1,Matsumpp2,p4int,nref)
!----------------------------------------------------------------------
!Do p1=1,npatch

p1=1+OMP_GET_THREAD_NUM()
...
if(nrep(j1)==0 .and. n1>n2)then

YDvertexre(j1)=-YDvertexre(i)                   ! real part reconstruction 
...
j2=imat(n1,n2,n3,n4,p1+j1*nprot,p2+j1*nprot,p3+j1*nprot)


if(nrep(j2)==0)then

YDvertexre(j2)=YDvertexre(i)                   ! real part reconstruction
...
j3=imat(n2,n1,n3,n4,p2+j1*nprot,p1+j1*nprot,p3+j1*nprot)


if(nrep(j3)==0)then

YDvertexre(j3)=-YDvertexre(i)                   ! real part reconstruction
!YDvertex(iasym+(NEQ/2))=-YDvertex(i+(NEQ/2))   ! imaginary part reconstruction
nrep(j3)=1
...
!----------------------------------------------------------------------
!----------------------------------------------------------------------
deallocate(p4intshadow,imatshadow)
!----------------------------------------------------------------------
