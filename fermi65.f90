!-----------------------------------------------------------------------------------------------------
! *****************--compared to fermi64old.f90, numerical group velocity- works fine
! *****************--interpolation for num dispersion
!-----------------------------------------------------------------------------------------------------
module commons
...
common eF,kxsamp,kysamp,dissamp,kyintmaxmat,kyintminmat&
       ,maxvertex,imat,start_time,irout,p4int,thetap,KFSpmat,imax!,Yvertex,imax
!------------------------------------------------------------------
...
character*4::phis
!------------------------------------------------------------------
! For the interpolation of dft dispersion
!------------------------------------------------------------------
DOUBLE PRECISION::dispana,BZconstsh,kysampmax,deltakxsamp,dispinterp
INTEGER::kyintmax,kyintmin,countmin,countmax
...
!-----------------------------------------------------------------------------------------------------
!-----------------------------------------------------------------------------------------------------
!open(unit=1,file="Aug1testa1r1b1np12freenumgroup.txt",status="new",action="write",position="rewind",IOSTAT=openstatus)
!-----------------------------------------------------------------------------------------------------
!-----------------------------------------------------------------------------------------------------
open(unit=2,file="dispersioncheck.txt",status="new",action="write",position="rewind",IOSTAT=openstatus)
!-----------------------------------------------------------------------------------------------------
!-----------------------------------------------------------------------------------------------------
...
!----------------------------------------------------------------------
!------------------------------------------------------------------
! Checking the interpolation for dispersion:
!------------------------------------------------------------------
!------------------------------------------------------------------
! Sample points:
!------------------------------------------------------------------
...
BZconstsh=BZconst!-(10.0d+00**(-7.0d+00))

kxsamp(kxint)=-BZconstsh+2.0d+00*BZconstsh*(kxint-1.0d+00)/(Ndis-1.0d+00)

!----------------------------------------------------------
deltakxsamp=2.0d+00*BZconstsh/(Ndis-1.0d+00)
!----------------------------------------------------------

kysamp(kyint)=-((sqrt(3.0d+00))/2.0d+00)*(BZconst)+(kyint-1)*2.0d+00*BZconstsh/(Ndis-1.0d+00)

!----------------------------------------------------------
if(kxsamp(kxint).LE. (-BZconstsh/2.0d+00))then

kysampmax=(sqrt(3.0d+00))*(kxsamp(kxint)+BZconstsh)

elseif(kxsamp(kxint).LE. (BZconstsh/2.0d+00))then

kysampmax=((sqrt(3.0d+00))/2.0d+00)*(BZconst)

elseif(kxsamp(kxint)>(BZconstsh/2.0d+00))then
...
if(kysamp(kyint).GE. -kysampmax .and. countmin==0)then
...
if(kysamp(kyint).GE. kysampmax .and. countmax==0)then
...
if(kxsamp(kxint) .GE. (-BZconst/2.0d+00) .and. kxsamp(kxint) .LE. (BZconst/2.0d+00))then
...
kyintminmat(kxint)=kyintmin
kyintmaxmat(kxint)=kyintmax

!------------------------------------------------------------------

if(kyintmin<1 .or. kyintmax>Ndis)then

print*,'out of bound',kyintmin,kyintmax,kxsamp(kxint)/BZconst,kysamp(kyint)/BZconst
...
if(l1 .LE. (nband/2.0d+00))then        

dispana=1.0*t*sqrt(3.0d+00+2.0d+00*(cos(Lconst*kpx*(sqrt(3.0d+00))))&
                +4.0d+00*(Cos(Lconst*kpx*(sqrt(3.0d+00))/2.0d+00))*(cos(Lconst*1.5d+00*kpy)))

else 

dispana=-1.0d+00*t*sqrt(3.0d+00+2.0d+00*(cos(Lconst*kpx*(sqrt(3.0d+00))))&
                +4.0d+00*(Cos(Lconst*kpx*(sqrt(3.0d+00))/2.0d+00))*(cos(Lconst*1.5d+00*kpy)))

endif

!------------------------------------------------------------------
dissamp(l1,kxint,kyint)=dispana
!------------------------------------------------------------------
end do !kyint
end do !kxint
end do !l1

print*,'checkingg outside:',dissamp(1,25,1),dissamp(1,26,1),dissamp(2,25,1)
!------------------------------------------------------------------
! Checking with ana:
!------------------------------------------------------------------
BZconstsh=BZconst-(10.0d+00**(-6.0d+00))
!------------------------------------------------------------------
Do l1=1,1!nband
Do kxint=1,Ndis
Do kyint=1,Ndis!!Ndis
!------------------------------------------------------------------

kpx=-BZconst+2.0d+00*BZconstsh*(kxint-1.0d+00)/(Ndis-1.0d+00)


if(kpx.LE. (-BZconst/2.0d+00))then

kpy= -(BZconstsh)*(sqrt(3.0d+00))*(kpx+BZconstsh)&
                      +2.0d+00*(BZconstsh)*(sqrt(3.0d+00))*(kpx+BZconstsh)*(kyint-1.0d+00)/(Ndis-1.0d+00)

elseif(kpx.LE. (BZconst/2.0d+00))then


kpy=(-(BZconstsh)*(sqrt(3.0d+00))/2.0d+00)+(BZconstsh)*(sqrt(3.0d+00))*(kyint-1.0d+00)/(Ndis-1.0d+00)

elseif(kpx>(BZconst/2.0d+00))then
...
dispinterp=dispersion(l1,KFS,angle,0.0d+00)

write(2,*)kpx/Bzconst,kpy/BZconst,dispana,dispinterp

print*,'dispersion comparison:',kxint,kyint,dispana-dispinterp,kyintminmat(kxint)&
                               ,kyintmaxmat(kxint),i,kpx/BZconst,kpy/BZconst



!------------------------------------------------------------------------------------------------------------------------------------
if(abs(dispana-dispinterp)>10.0d+00**(-1.0d+00))then

!print*,'Ups! dispersion does not fit!',kxint,kyint,dispana-dispersion(l1,KFS,angle,0.0d+00)
...
DOUBLE PRECISION::px0,px1,px2,px3,px4,px2fold,px3fold,&
                  ...
                  anglep1,anglep2,anglep3,KFSp1,KFSp2,KFSp3
...
kyintmin=kyintminmat(xint)
!------------------------------------------------------------------

if(kx .GE. (-BZconst/2) .and. kx .LE. (BZconst/2.0d+00))then
kyintmin=1
endif
...
if(yint==kyintmaxmat(xint)-1)then
...
elseif(yint<kyintmaxmat(xint))then
...
if(county>Ndis)then
print*,'I ma stuck in loop-y!',kx/BZconst,ky/BZconst,xint,kyintmin,kyintmaxmat(xint),kxsamp(xint)/BZconst,kxsamp(xint+1)/BZconst
endif
...
!CALL Hexagonal(px2,py2,idp2)

!if(idp2==0)tthen

!anglep2=atan2(px2,py2)
!anglep2=(-SIGN(1.0d+00,anglep2)+1.0d+00)*(pi)+anglep2

!CALL BZ_edge(anglep2,KFSp2)

!px2=KFSp2*cos(anglep2)
!py2=KFSp2*sin(anglep2)
...
px0=kxsamp(xint)
py0=kysamp(yint)
pz0=dissamp(blab,xint,yint)-energy

px4=kxsamp(xint+1)
py4=kysamp(yint+1)
pz4=dissamp(blab,xint+1,yint+1)-energy
!-------------------------------------
! Checking if ps are inside
!-------------------------------------
CALL Hexagonal(px0,py0,idp0)
CALL Hexagonal(px1,py1,idp1)
CALL Hexagonal(px2,py2,idp2)
CALL Hexagonal(px3,py3,idp3)
CALL Hexagonal(px4,py4,idp4)
!-------------------------------------
! Begin fold:
!-------------------------------------
if(idp3==0)then
...
CALL Folding(px3,py3,px3fold,py3fold)
...
if(xintp3fold==Ndis-1)then

exit

elseif(px3fold .GE. kxsamp(xintp3fold) .and. px3fold< kxsamp(xintp3fold+1)) then

exit

elseif(xintp3fold<Ndis)then

xintp3fold=xintp3fold+1
...
if(countx>Ndis)then
print*,'I ma stuck in loop-x-p3fold!'
endif

end do

!------------------------------------------------------------------
kyintmin=kyintminmat(xintp3fold)
!------------------------------------------------------------------

if(px3fold .GE. (-BZconst/2) .and. px3fold .LE. (BZconst/2.0d+00))then
kyintmin=1
endif
...
if(yint==kyintmaxmat(xintp3fold)-1)then

exit

elseif(py3fold .GE. kysamp(yintp3fold) .and. py3fold< kysamp(yintp3fold+1)) then

exit

elseif(yintp3fold<kyintmaxmat(xintp3fold))then

yintp3fold=yintp3fold+1
...
if(county>Ndis)then
print*,'I ma stuck in loop-y-p3-fold!'
endif

end do


pz3=dissamp(blab,xintp3fold,yintp3fold)-energy

call Hexagonal(kxsamp(xintp3fold),kysamp(yintp3fold),i)

if(i==0)then

print*,'p3fold is also outside BZ'

endif

!print*,'check-p3-fold:',px3fold/BZconst,kxsamp(xintp3fold)/BZconst,py3fold/BZconst,kysamp(yintp3fold)/BZconst
...
dispersion=pz1-(nx/nz)*(kx-px1)-(ny/nz)*(ky-py1)

else


print*,'ERROR!!!!',kx/BZconst,ky/BZconst,Ax,By,Ay,Bx,xint,yint,kyintmaxmat(xint)
...
dispersion=dissamp(blab,xint,yint)-energy

endif

!print*,'checking_p=',xint,yint,kyintminmat(xint),kyintmaxmat(xint),kx/BZconst,ky/BZconst,kxsamp(xint)/BZconst,kysamp(yint)/BZconst
!print*,'checking_p1=',px1/BZconst,py1/BZconst,idp1
!print*,'checking_p2=',px2/BZconst,py2/BZconst,idp2
!print*,'checking_p3=',px3/BZconst,py3/BZconst,idp3,kxsamp(xintp3fold)/BZconst,kysamp(yintp3fold)/BZconst
!print*,'checking_p4=',px4/BZconst,py4/BZconst,idp4

!print*,'checking=',pz1,pz2,pz3,xint,yint,max(abs(pz1-pz2),abs(pz1-pz3),abs(pz2-pz3))
!------------------------------------------------------------------
...
deltakx=1.0d+00*pi/(10.0d+00**(8.0d+00))
deltaky=1.0d+00*pi/(10.0d+00**(8.0d+00))
